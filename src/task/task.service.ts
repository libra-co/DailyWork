 ...
 * @LastEditTime: 2023-06-26 16:48:50
    ...
    async update(user: User, updateTaskDto: UpdateTaskDto): Promise<CommonResult> {
        ...
        if ('parentTaskId' in updateTaskDto) {
            ...
            if (isMoveWithChildren) {
                ...
                if (currentChildrenTasks.length) {
                    try {
                        const result = await this.prismaService.$transaction(async (prisma) => {
                            ...
                            const targetTaskIndex = targetParentTask ? projectColumns.findIndex(column => column.columnId === targetParentTask.columnId) : -1
                            const childrenTaskData = await generateChildrenTaskData(currentTaskTree, targetTaskIndex)
                        })
                ...
                if (currentChildrenTasks.length) {
                    try {
                        ...
                        const result = await this.prismaService.$transaction(async (prisma) => {
                            /**
                             * 
                             * @param tasks 当前任务带子任务
                             * @param index 父任务移动前的位置,第一个子任务将会用当前位置的columnId
                             */
                            const generateChildrenTaskData = async (currentParentTaskId: string, tasks, index: number) => {
                                await Promise.all(tasks.map(async task => {
                                    ...
                                    if (task.children) {
                                        return generateChildrenTaskData(task.parentTaskId, task.children, index++)
                                    }
                                    return task
                                })
                                )
                            }
                            console.log('projectColumns', task, projectColumns, targetTaskIndex, projectColumns[targetTaskIndex + 1])
                            if (!projectColumns[targetTaskIndex + 1]) {
                                const newColumn = await this.prismaService.column.create({
                                    data: {
                                        projectId: task.projectId,
                                        columnName: `模块${projectColumns.length + 1}`,
                            ...
                            await prisma.task.update({
                                ...
                                data: {
                                    ...
                                    columnId: projectColumns[targetTaskIndex + 1].columnId
                                }
                            })
                            const childrenTaskData = await generateChildrenTaskData(task.parentTaskId, currentTaskTree.children, projectColumns.findIndex(column => column.columnId === task.columnId))
                        }
                        )
                        await this.columnService.checkAndDeleteEmptyColumn(task.projectId)
                        return successReturn
                ...
                // 如果移动时,任务没有子任务,则直接移动
                try {
                    const targetParentTaskColumnIndex = targetParentTask ? projectColumns.findIndex(column => column.columnId === targetParentTask.columnId) : -1
                    // 是否需要创建新的列
